...
<template>
  <div>
    ...
    <a-row :gutter="20">
      <a-col :xs="24" :xl="14"><a-card title="中转服务" class="panel-card" :loading="loading"><a-descriptions v-if="data" :column="1" bordered size="small"><a-descriptions-item label="服务状态"><a-tag color="success">正常</a-tag></a-descriptions-item><a-descriptions-item label="中转地址">{{ data.service.relay_host }}:{{ data.service.relay_port }}</a-descriptions-item><a-descriptions-item label="加密方式">{{ data.service.relay_method }}</a-descriptions-item><a-descriptions-item label="活跃刷新令牌">{{ data.active_refresh_tokens }}</a-descriptions-item></a-descriptions></a-card></a-col>
