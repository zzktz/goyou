...
<template>
  <div>
    ...
    <a-row :gutter="20">
      <a-col :xs="24" :xl="14"><a-card title="中转服务" class="panel-card" :loading="loading"><a-descriptions v-if="data" :column="1" bordered size="small"><a-descriptions-item label="服务状态"><a-tag :color="relayStatusColor(data.relay?.status)">{{ relayStatusLabel(data.relay?.status) }}</a-tag></a-descriptions-item><a-descriptions-item label="sing-box 进程"><a-tag :color="data.relay?.process_running ? 'success' : 'error'">{{ data.relay?.process_running ? '运行中' : '未运行' }}</a-tag></a-descriptions-item><a-descriptions-item label="心跳状态"><span>{{ data.relay?.heartbeat_stale ? '已超时' : '正常' }} · {{ formatDate(data.relay?.heartbeat_at) }}</span></a-descriptions-item><a-descriptions-item label="端口冲突"><a-tag :color="data.relay?.duplicate_ports?.length ? 'error' : 'success'">{{ data.relay?.duplicate_ports?.length ? formatPorts(data.relay.duplicate_ports) : '无' }}</a-tag></a-descriptions-item><a-descriptions-item label="未监听租约端口"><a-tag :color="data.relay?.missing_ports?.length ? 'warning' : 'success'">{{ data.relay?.missing_ports?.length ? formatPorts(data.relay.missing_ports) : '无' }}</a-tag></a-descriptions-item><a-descriptions-item label="监听端口">{{ formatPorts(data.relay?.listen_ports) }}</a-descriptions-item><a-descriptions-item label="中转地址">{{ data.service?.relay_host }}:{{ data.service?.relay_port_start }}-{{ data.service?.relay_port_end }}</a-descriptions-item><a-descriptions-item label="加密方式">{{ data.service?.relay_method }}</a-descriptions-item><a-descriptions-item label="活跃刷新令牌">{{ data.active_refresh_tokens || 0 }}</a-descriptions-item></a-descriptions><a-alert v-if="data?.relay?.error" type="warning" show-icon :message="data.relay.error" class="relay-health-alert" /></a-card></a-col>
