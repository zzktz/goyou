...
import { AppstoreOutlined, CloudServerOutlined, DownOutlined, MenuFoldOutlined, MenuUnfoldOutlined, SettingOutlined, TeamOutlined, LogoutOutlined, FileTextOutlined, RocketOutlined } from '@ant-design/icons-vue'
...
const active = computed(() => route.path.startsWith('/users') ? '/users' : route.path.startsWith('/leases') ? '/leases' : route.path.startsWith('/traffic-logs') ? '/traffic-logs' : route.path.startsWith('/releases') ? '/releases' : route.path)
  ...
  <a-layout class="shell">
    <a-layout-sider v-model:collapsed="collapsed" class="sider" :width="224" :collapsed-width="64" collapsible :trigger="null">
      ...
      <a-menu theme="dark" mode="inline" :selected-keys="[active]" @click="({ key }) => go(key)">
        ...
        <a-menu-item key="/releases"><RocketOutlined /><span>版本发布</span></a-menu-item>
        <a-menu-divider />
